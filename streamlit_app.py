...
def upload_to_drive(file_content, file_name):
    ...
    folder_id = '1EHPRmig_vFpRS8cgz-8FsG88_LhT_JY5' # 您的雲端資料夾 ID
...
def create_single_pdf(title, data_dict, canvas_key):
    ...
    for k, v in data_dict.items():
        ...
        pdf.multi_cell(0, 10, txt=f"{k}: {safe_v}")
...
st.markdown("""
    ...
    .stButton>button { width: 100%; height: 3.5em; background-color: #2E7D32; color: white; border-radius: 5px; }
    </style>
...
if st.session_state.current_page == "1. 施工安全危害告知單":
    ...
    with st.container(border=True):
        ...
        with col1:
            comp = st.text_input("承攬商名稱", value=st.session_state.get('company',''), key="main_comp")
            worker = st.text_input("施作人員姓名", value=st.session_state.get('worker_name',''), key="main_worker")
        with col2:
    ...
    with st.container(border=True):
        ...
        hazards = st.multiselect("勾選本次作業危害項目", ["墜落", "感電", "物體飛落", "火災爆炸", "交通事故", "缺氧窒息", "化學品接觸", "捲入夾碎"], default=st.session_state.get('selected_hazards', []))
...
# --- 第二頁：工具箱會議 ---
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            co_comp = st.text_input("共同作業廠商", key="tool_co_comp")
            content = st.text_area("工程內容", key="tool_content")
        with col2:
            st.number_input("總作業人數", min_value=1, step=1, key="tool_num")
    ...
    if st.button("確認提交工具箱會議並獨立存檔"):
        with st.spinner("存檔中..."):
            filename = f"02_Toolbox_{st.session_state.get('company','')}_{date.today()}.pdf"
            pdf_bytes = create_single_pdf("02_Toolbox_Meeting", {"Company": st.session_state.get('company',''), "Content": content}, "sign_toolbox")
            if upload_to_drive(pdf_bytes, filename):
                st.success(f"✅ 工具箱會議表存檔成功：{filename}")
                st.toast("工具箱會議已送出")
                ...
                elif any(x in haz for x in ["墜落", "感電", "缺氧窒息", "物體飛落"]):
...
# --- 第三頁：動火單 (保留 17 項) ---
    ...
    for idx, item in enumerate(check_items):
        ...
        c2.checkbox("OK", key=f"fire_check_{idx}")

    st_canvas(stroke_width=3, background_color="#eee", height=150, key="sign_fire")
    
    if st.button("完成提交動火許可"):
        with st.spinner("動火許可證上傳中..."):
            filename = f"03_Fire_{st.session_state.get('company','')}_{date.today()}.pdf"
            pdf_bytes = create_single_pdf("03_Hot_Work_Permit", {"Status": "Verified"}, "sign_fire")
            if upload_to_drive(pdf_bytes, filename):
                st.success(f"✅ 動火許可證已存檔：{filename}")
...
# --- 第四頁：特殊危害 ---
    ...
    st.subheader("勾選特殊作業項目")
    col1, col2 = st.columns(2)
    with col1:
        st.checkbox("局限空間作業")
        st.checkbox("吊掛作業")
        st.checkbox("高架作業 (1.8M以上)")
    with col2:
        st.checkbox("危險管路拆卸")
        st.checkbox("臨時用電作業")
    ...
    if st.button("完成提交特殊作業許可"):
        with st.spinner("最後一份表單存檔中..."):
            filename = f"04_Special_{st.session_state.get('company','')}_{date.today()}.pdf"
            pdf_bytes = create_single_pdf("04_Special_Work_Permit", {"Result": "Complete"}, "sign_spec")
            if upload_to_drive(pdf_bytes, filename):
                st.success(f"✅ 特殊作業許可證已存檔：{filename}")
                st.balloons()
                st.session_state.current_page = "1. 施工安全危害告知單"
                st.rerun()
