...
if st.session_state.current_page == "1. 施工安全危害告知單":
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            st.text_input("共同作業廠商", key="co_comp")
        with col2:
            st.number_input("總作業人數", min_value=1, step=1, key="workers_count")
            st.text_area("工程內容", key="eng_content")
    st.subheader("✍️ 施工人員簽名確認")
    st_canvas(stroke_width=3, background_color="#eee", height=200, key="sign_workers")
...
# --- 頁面 3：動火作業許可證 (本次大改區) ---
    ...
    with st.container(border=True):
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.text_input("動火設備", placeholder="例如：電焊機、乙炔切割器", key="fire_tool")
            st.text_input("連絡電話", key="fire_phone")
        with col2:
            st.write("**作業期間 (動火作業一日為限)**")
            c1, c2, c3 = st.columns([2, 1, 1])
            work_day = c1.date_input("日期", value=date.today())
            start_h = c2.number_input("起(時)", 0, 23, 8)
            end_h = c3.number_input("迄(時)", 0, 23, 17)
            st.caption(f"核准時間：{work_day} {start_h}:00 ~ {end_h}:00")

    st.subheader("✅ 動火檢查表 (勾選結果)")
    
    # 建立檢查表標題欄
    h_col1, h_col2, h_col3, h_col4 = st.columns([4, 1, 1, 1])
    h_col1.markdown("**檢查重點**")
    h_col2.write("承攬商")
    h_col3.write("監工")
    h_col4.write("環安")
    ...
    for idx, item in enumerate(check_items):
        ...
        c2.checkbox("", key=f"c_vendor_{idx}", label_visibility="collapsed")
        c3.checkbox("", key=f"c_sup_{idx}", label_visibility="collapsed")
        c4.checkbox("", key=f"c_safety_{idx}", label_visibility="collapsed")
    ...
    with sig_col1:
        st.write("施工單位簽名")
        st_canvas(stroke_width=3, background_color="#fafafa", height=120, key="sign_fire_vendor")
    with sig_col2:
        st.write("監工單位簽名")
        st_canvas(stroke_width=3, background_color="#fafafa", height=120, key="sign_fire_sup")

    if st.button("完成動火許可提交"):
        st.success("動火作業許可證已存檔！")
        st.session_state.current_page = "1. 施工安全危害告知單"
        st.rerun()
